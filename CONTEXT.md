...
| Phase 2.8.4 Zigbee Mesh Slice | ✅ Complete | [`capabilities/base.py`](capabilities/base.py) (catalogue 23 → 26: append `zigbee.discovery.{scan,inspect,join}` after USB; `join` SAFE_ACTIVE `mutates_state=True` + `produces_evidence=True`; `KNOWN_DOMAINS` 8 → 9 +`zigbee`), [`simulator/entities.py`](simulator/entities.py) (`ZigbeeNetwork` pan_id/extended_pan_id/channel/rssi/prefix/node_count + `ZigbeeNode` ieee_address/short_address/role/network/parent_short_address/lqi/joined), [`simulator/environment.py`](simulator/environment.py) (`zigbee_networks` + `zigbee_nodes` + snapshot), [`simulator/scenarios.py`](simulator/scenarios.py) (lab full mesh PAN `0x1A2B` — coord/`0x0000`, router/`0x0001`→`0x0000`, lamp/`0x0002`→`0x0001`, UNJOINED join target `...:04`; home PAN `0x2C3D`), [`simulator/simulator.py`](simulator/simulator.py) (`_zigbee_network_entities` recomputes node_count LIVE from joined nodes, `_zigbee_node_entities`, `action_zigbee_scan`/`inspect` PASSIVE + `action_zigbee_join` SAFE_ACTIVE with per-target gates + `_zigbee_next_short`/`_zigbee_parent`; HANDLERS registration → zigbee `supported=True`; `zigbee_joined:` note-prefix mapping), [`core/evidence.py`](core/evidence.py) (`KNOWN_EVIDENCE_KINDS` 9 → 10: +`zigbee_join`), [`engine/runner.py`](engine/runner.py) (`zigbee_workflow_plan()` 4 actions → 1 `zigbee_join` evidence), [`cli/phase2.py`](cli/phase2.py) (`--plan zigbee_workflow`), [`tests/test_phase284_zigbee.py`](tests/test_phase284_zigbee.py) (51 tests — scan/inspect/join success + per-target prereq + unknown PAN/node + already-joined + malformed args + no-false-positive + provenance + determinism + JSONL `evidence.created` (1; 0 under PASSIVE) + HTML evidence section + TUI forward-guard + Phase 2.7/2.8.0/2.8.1/2.8.2/2.8.3 regression); count/kinds/producers guards bumped 23→26 across 8 prior suites; [`suryafool-cli/src/components/EvidenceFeed.test.js`](suryafool-cli/src/components/EvidenceFeed.test.js) (+2 Zigbee-format assertions, forward-linked from Python). Reports/JSONL/TUI reuse the generic evidence pipeline — no report/event/TUI code changes. No LLM/hardware/RF replay/policy/evidence-system changes. 15 Python suites (**523 tests**) + 60 JS tests + esbuild + 2 Node smokes all green. **Phase 2.8.4 COMPLETE; roadmap 2.8.5–2.8.9 pending.** |
