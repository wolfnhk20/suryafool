...
- `KNOWN_DOMAINS: frozenset[str]` — open-set vocabulary of the eight intended domains. **Not enforced** — the catalogue may add new ones later.
...
- Python: **13 suites / 431 tests**, all green (`test_phase2_core` 21, `test_phase26_authorization` 14, `test_phase27_active_sim` 34, `test_phase271` 34, `test_phase272` 35, `test_phase273` 48, `test_phase275` 37, `test_phase276` 31, `test_phase277` 30, `test_phase279` 26, `test_phase280_multidomain` 29, `test_phase281` 39, `test_phase282_nfc_read` 53).
...
## Phase 2.8 — Roadmap (2.8.0 declared COMPLETE)

Established: multi-domain catalogue surface (21 entries) on the FROZEN Phase 2.7 stack; registered-but-unsupported semantics (harmless policy REJECT before provider); `IrSignal` / `EthernetHost` / `UsbDevice` entity substrate + empty `Environment.ir/ethernet/usb`; CLI `capabilities` `supported` field.
...
| 2.8.4 | Ethernet / Network — `discover`/`inspect` handlers on `EthernetHost` | ethernet entries become supported |
| 2.8.5 | USB — `enumerate`/`inspect` handlers on `UsbDevice` | usb entries become supported |
| 2.8.6 | Cross-domain deterministic workflows (multi-protocol plans) | — |
| 2.8.7 | Evidence/analysis generalization across new domains | new evidence kinds when real producers exist |
| 2.8.8 | Phase 2.8 integration / freeze | contract audit |
