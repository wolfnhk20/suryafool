...
async function main() {
  ...
  const yargInstance = yargs(hideBin(process.argv))
    ...
    .command('capabilities', 'List available capabilities (Phase 2)')
    .command('scenarios', 'List predefined simulator scenarios (Phase 2)')
    .command('providers', 'List available backends/providers (Phase 2.5)')
    .command('run', 'Execute a deterministic simulator run (Phase 2)')
    .command('show <run_id>', 'Show a stored run (Phase 2)')
    .command('report <run_id>', 'Regenerate the HTML report for a run (Phase 2)')
    .help(false)
    .version(false)
    .exitProcess(false)
    .strict()
